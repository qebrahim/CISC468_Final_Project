...
def handle_response(conn, addr, payload):
    ...
    try:
        ...
        print()
        print(f"Do you want to verify and trust this peer? (y/n): ", end="", flush=True)
        ...
        store_verification_request(peer_id, temp_contact, challenge_id, signature, conn)
...
def handle_verify(conn, addr, payload):
    ...
    try:
        ...
        logger.info(f"Received VERIFY from peer {peer_id} with status: {status}")
        ...
        if status == "verified":
            ...
            logger.info(f"Peer {peer_id} is considering our authentication request")
...
def handle_trust(conn, addr, payload):
    ...
    try:
        ...
        if peer_id is None or trusted is None:
            logger.error("Missing data in TRUST message")
            conn.sendall(b"ERR:INVALID_TRUST_MESSAGE")
            return
...
def store_verification_request(peer_id, contact_data, challenge_id, signature, conn):
    ...
    logger.info(f"Stored verification request from peer {peer_id} for user confirmation")
...
def process_verification_response(peer_id, trusted):
    ...
    try:
        if trusted:
            ...
            valid = authentication.verify_signature(peer_id, challenge_id, signature)
            ...
            if valid:
                ...
                try:
                    conn.sendall(f"AUTH:TRUST:{json.dumps(trust_msg)}".encode('utf-8'))
                    logger.info(f"Notified peer {peer_id} that they are now trusted")
                except:
                    logger.warning(f"Could not notify peer {peer_id} - connection may be closed")
                ...
                print(f"Peer {peer_id} verified and added to trusted contacts!")
                
            else:
                # Invalid signature
                print(f"Warning: Signature verification failed for peer {peer_id}!")
...
def handle_auth_response(sock, peer_address, payload):
    ...
    try:
        ...
        store_verification_request(peer_id, temp_contact, challenge_id, signature, sock)
