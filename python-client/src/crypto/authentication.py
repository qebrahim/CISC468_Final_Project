...
class PeerAuthentication:
    ...
    def verify_signature(self, peer_id, challenge_id, signature_b64):
        ...
        try:
            ...
            # For initial authentication, we don't have the contact yet
            # We need to get the public key from the stored verification request
            from crypto import auth_protocol
            pending_requests = auth_protocol.get_pending_verifications()

            if peer_id in pending_requests:
                # Get public key from the pending request
                contact_data = pending_requests[peer_id]["contact_data"]
                public_key_pem = contact_data["public_key"]
                ...
                # Fall back to checking if they're already a trusted contact
