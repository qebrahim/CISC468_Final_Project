...
class KeyMigration:
    ...
    def initiate_migration(self):
        ...
        try:
            ...
            backup_dir = self.storage_path / 'backup' / datetime.now().strftime('%Y%m%d_%H%M%S')
            ...
            for peer_id, contact in contacts.items():
                try:
                    ...
                    if not addr or ":" not in addr:
                        logger.warning(f"Invalid address for contact {peer_id}")
                        failure_count += 1
                        continue
                    ...
                    # Send notification
                    sock.sendall(f"MIGRATE_KEY:{notification_json}".encode('utf-8'))
                    ...
                    try:
                        ...
                        if response.startswith("MIGRATE_KEY_ACK:"):
                            ...
                            if status == "accepted":
                                print(f"Contact {peer_id} accepted key migration")
                                success_count += 1
                            else:
                                print(f"Contact {peer_id} rejected key migration: {status}")
                                failure_count += 1
                        else:
                            print(f"Unexpected response from {peer_id}: {response[:20]}")
                            failure_count += 1
...
def handle_key_migration(message, addr, conn, contact_manager, authentication):
    ...
    try:
        ...
        if contact.get("public_key") != old_public_key:
            logger.error(f"Old public key doesn't match stored key for peer {peer_id}")
        ...
        if not verify_migration_signature(message_text, signature, old_public_key):
            logger.error(f"Invalid signature in key migration from peer {peer_id}")
            send_migration_ack(conn, authentication.peer_id, "invalid_signature")
            return False
        ...
        try:
            send_migration_ack(conn, authentication.peer_id if authentication else "unknown", "error")
        except:
            pass
        return False
